...
func (r *router) findRoute(method string, path string) (*matchInfo, bool) {
	...
	if path == "/" {
		return &matchInfo{root, nil}, true
	}
...
func (n *node) childOrCreate(path string) *node {
	...
	if path[0] == ':' {
		...
		if isReg {
			return n.childCreateReg(path, expr, paramName)
		}
		return n.childCreateParam(path, paramName)
		/*		if n.starChild != nil {
...
func (n *node) childCreateReg(path string, expr string, paramName string) *node {
	if n.starChild != nil {
	...
	if n.regChild != nil {
		if n.paramName != paramName || n.regChild.regExpr.String() != expr {
...
func (n *node) childCreateParam(path string, paramName string) *node {
	if n.regChild != nil {
